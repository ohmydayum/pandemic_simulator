...
function _deepClone(obj) {
  ...
  function run() {
    ...
    fill(100);
    noStroke();
    text(_CREDIT, config.simulation.CANVAS_WIDTH - _CREDIT.length * 5, config.simulation.CANVAS_HEIGHT + INFO_SIZE);
    fill(255, 200);
    stroke(0);
    rect(110, config.simulation.CANVAS_WIDTH + 15, config.simulation.CANVAS_HEIGHT - 103, H, 5);
  ...
  function create_buttons() {
    ...
    copy_sharable_link_button.setAttribute("data-content","Sharable link copied to clipboard!");
    copy_sharable_link_button.addEventListener("click", copy_sharable_link);
    buttons_div.appendChild(copy_sharable_link_button);
  }
  ...
  function draw_counters(counters) {
    fill(255, 200);
    stroke(0);
    rect(5, config.simulation.CANVAS_HEIGHT + 15, 100, 5 + 15 * Object.keys(COLORS).length, 5);
    noStroke();
    textSize(12);
    i = 1;
    for (var key in counters) {
      fill(COLORS[key]);
      text(key + ": " + counters[key], 10, config.simulation.CANVAS_HEIGHT + 15 + 15 * i++);
    }
  }
  ...
  function draw_graph(counters) {
    ...
    y_top = config.simulation.CANVAS_HEIGHT + 15+300;
    for (key in counters) {
      fill(COLORS[key]);
      rect(x, y_top, 200 / config.simulation.GRAPH_DURATION, counters[key] / population.length * H);
      y_top += counters[key] / population.length * H;
    }
    rect(x, (1 - config.world.HEALTHCARE_CAPACITY - counters["quarantine"] / population.length) * H + config.simulation.CANVAS_HEIGHT + 15, 200 / config.simulation.GRAPH_DURATION, 1);
  }
  
  function draw_config_changes_act() {
    noStroke();
    ind+=2;
  }